...
void setup_pwm()
{
    ...
    slice_led = pwm_gpio_to_slice_num(LED);    // Obtém o slice do PWM associado ao pino do LED
    pwm_set_enabled(slice_led, false);        // Desabilita PWM antes de reconfigurar
    pwm_set_clkdiv(slice_led, DIVIDER_PWM);    // Define o divisor de clock do PWM
    pwm_set_wrap(slice_led, PERIOD);           // Configura o valor máximo do contador (período do PWM)
