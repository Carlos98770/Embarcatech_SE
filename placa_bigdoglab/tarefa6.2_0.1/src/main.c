...
char *texto[] = {"OPCAO 1","OPCAO 2","OPCAO 3","Joystick LED","BUZZER","LED RGB"};
...
int len(const char *texto){   //Função para saber o tamanho de uma string
  int tam = 0;
...
void menu(char *texto[]) {      //Função que desenha o menu no OLED
...
void opcoes(){        //Função que defini qual a opção será ativa de acordo com o a variavel state_menu
    ...
    switch (state_menu) {
        case 0: programa_1(); break;
        case 1: programa_2(); break;
        case 2: programa_3(); break;
    }
...
void linha_dinamica(int meiob, int meiof, int offset){    //Função responsavel por desenhar a linha dinamica de acordo com movimento do joystick
