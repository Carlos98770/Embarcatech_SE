...
int VRX = 26;          // Pino de leitura do eixo X do joystick (conectado ao ADC)
...
const float DIVIDER_PWM_j = 16.0;          // Divisor fracional do clock para o PWM
const uint16_t PERIOD_j = 4096;            // Período do PWM (valor máximo do contador)
...
void setup_pwm_led(uint led, uint *slice, uint16_t level)
{
  ...
  *slice = pwm_gpio_to_slice_num(led);   // Obtém o slice do PWM associado ao pino do LED
  pwm_set_enabled(*slice, false);        // Desabilita PWM antes de reconfigurar
  pwm_set_clkdiv(*slice, DIVIDER_PWM_j);   // Define o divisor de clock do PWM
  pwm_set_wrap(*slice, PERIOD_j);          // Configura o valor máximo do contador (período do PWM)
